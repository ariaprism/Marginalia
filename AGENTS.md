...
- [ ] 在干净浏览器取回现有一册书的书目、17 章、阅读状态与 4 笔痕迹
- [ ] 接通私有文件传输、触发调度与完整恢复
...
1. 在另一个干净浏览器中登录并取回书目与上述结构化内容，确认本地有内容和云端有内容时都不会互相误盖。
2. 定向验证两台设备同时修改折页／批注以及完整删除时的时间决胜与不复活规则。
3. 上传和下载私有 EPUB／封面；随后接应用启动、网络恢复、回到前台、写入防抖和 Realtime 提醒。
5. 最后启用“立即收好”和“从云端恢复”，完成真实跨设备验收后形成 `v0.3 Cloud Ink`。
6. “念头”先保留说明壳，等明确它是独立随笔、待读入口还是书内痕迹聚合后再实现；“来访”以后直接消费真实 MCP session / visit events，不先制造假记录。

...
- [x] 整理待寄箱与首次连接保护：同对象只保留最新动作，云端书房按书／正文／阅读状态／痕迹显示人话摘要，新设备先取云端再合并以免空白名帖覆盖旧数据

## 正在进行

- [ ] 用一册真实 EPUB 完成首次人工往返：同章章首／章中／章末批注 → 导出给小鱼 → 导入回复 → 刷新恢复
- [ ] 接通真实 Supabase push／pull、文件传输、触发调度与云端书房状态

## 下次开工：直接从这里继续

当小狐狸说“小G，我们接下来干嘛”时，无需重新盘视觉稿，也不要从已经完成的交换契约、同步语义或建库工作重新开始。直接按下面顺序推进：

1. 在正式站确认“待收内容”已从原始操作数变为书房摘要，并再点一次“先收名帖与书目”完成首次保护标记。
2. 依次接入章节、阅读位置、折页、划线、批注、页边文字和完整删除传播，并验证离线重试与两台设备同时修改时的规则。
3. 在另一个干净浏览器中取回书目与上述结构化内容，确认本地有内容和云端有内容时都不会互相误盖。
4. 上传和下载私有 EPUB／封面；随后接应用启动、网络恢复、回到前台、写入防抖和 Realtime 提醒。
...
- 登录后可手动执行“先收名帖与书目”：它会真实 push／pull 名帖和书籍元数据，重复操作不会生成副本；EPUB、章节与痕迹仍不会被确认出队。
- 完整“立即收好”和“从云端恢复”仍未启用；文件上传下载、其余实体和恢复验收完成前不能暗示整间书房已经备份。
...
- 2026-09-02：待寄箱改为同实体写入时替换旧动作，并在云端书房打开时整理历史重复项；新增按书、原书、正文、阅读状态与痕迹的人话摘要。名帖／书目首次同步改为先拉后合并，默认空白名帖让位于云端，真实本地编辑仍按时间决胜；定向 15 项、`npm run build`、`npm run lint` 通过。全量 127 项中 126 项通过，唯一失败为既有书房进入阅读器用例 20 秒超时，随后单独复跑通过（1.55 秒）。

...
2. 接通最小的真实往返：登录后把名帖和一本书的资料送入 Supabase，再在另一个干净浏览器中取回并显示；重复操作不能生成副本。
3. 依次接入章节、阅读位置、划线、批注、页边文字和删除传播，并验证离线重试与两台设备同时修改时的规则。
...
- 登录只表示云端认出了当前身份，不表示本地书、批注或 EPUB 已经上传。
- “立即收好”和“从云端恢复”目前故意禁用；真实 push／pull、文件上传下载和恢复保护通过验收后才启用，不能提供一个看似成功但实际没有同步的按钮。
- 在按钮启用前，本地阅读和写批注仍然安全地保存在当前浏览器；卸载浏览器数据或换设备不会自动恢复。

...
1. 先补齐真实同步前的本地遗漏：折页需要能单独同步；名帖不能只留在 `localStorage`；旧版本中已经存在的藏书第一次登录时也要能加入待上传清单。
2. 在正式站用现有书房点一次“先收名帖与书目”，确认云端真实入库；随后补首次合并保护，并在另一个干净浏览器中取回显示。
3. 依次接入章节、阅读位置、划线、批注、页边文字和完整删除传播，并验证离线重试与两台设备同时修改时的规则。
...
- 完整“立即收好”和“从云端恢复”仍未启用；文件上传下载、其余实体、首次合并保护和恢复验收完成前不能暗示整间书房已经备份。

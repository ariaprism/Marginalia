...
- [ ] 在正式站用现有一册书人工验收结构化内容寄出，并在干净浏览器取回
- [ ] 接通私有文件传输、触发调度与完整恢复
...
1. 在正式站刷新后点击“收好书页与痕迹”，确认正文、阅读状态和痕迹寄出，待收摘要只剩 EPUB 原书。
2. 在另一个干净浏览器中登录并取回书目与上述结构化内容，确认本地有内容和云端有内容时都不会互相误盖。
3. 定向验证两台设备同时修改折页／批注以及完整删除时的时间决胜与不复活规则。
